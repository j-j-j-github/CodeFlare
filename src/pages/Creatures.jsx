...
function CreatureCard({ creature, index, onClick }) {
  return (
    <motion.div
      ...
      role="button"
      tabIndex={0}
      aria-label={`Learn more about ${creature.name}`}
      onKeyDown={(e) => { if (e.key === 'Enter') onClick(creature); }}
    >
      {/* Image */}
      <div className="relative h-52 overflow-hidden bg-[var(--color-bg-secondary)]">
        ...
        <div className="absolute bottom-3 left-4 right-4">
          ...
          <p className="font-serif text-sm italic text-[var(--color-muted)] font-normal">{creature.title}</p>
        </div>
      </div>

      {/* Content */}
      <div className="p-5 flex flex-col justify-between flex-grow">
        <p className="text-xs text-[var(--color-muted)] leading-relaxed line-clamp-3 mb-4 font-light">{creature.description}</p>
        ...
        <div className="pt-3 border-t border-[var(--color-gold)]/10 flex items-center justify-between text-[var(--color-gold)] group-hover:text-[var(--color-gold-bright)] transition-all duration-300">
          <span className="tracking-label text-[0.6rem] font-semibold">EXPLORE BEING</span>
  ...
  return (
    <div className="pt-28 pb-24 px-6">
      <div className="max-w-[1400px] mx-auto">
        <SectionHeader
          eyebrow="LEGENDARY BEINGS"
          title="MYTHICAL CREATURES"
          subtitle="Explore Nagas, Garuda, Rakshasas, Yakshas, and aquatic Makaras across Indian traditions."
        />

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {creatures.map((creature, index) => (
            <CreatureCard key={creature.id} creature={creature} index={index} onClick={setSelected} />
          ))}
        </div>
      </div>
